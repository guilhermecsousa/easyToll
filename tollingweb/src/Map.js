            ...
            .then((data) => {
                data.pos.map((d)=>{
                    ...
                    el.addEventListener('click', () => { 
                        ...
                        //Fetch info of transactions of rsu=d.station_id
                        fetch('http://localhost:8000/trans?rsu=' + d.station_id)
                        .then(response2 => response2.json())
                        .then((data2) => {
                            this.setState({trans:data2.aux})
                        });
                    });
        ...
        if(this.state.selected==null){
                ...
                <div>
                    <div>
                        <ReactBoot.Navbar bg="light" expand="md">
                            <FaWifi style={{marginLeft:'2%',marginBottom:'0.6%'}}/>
                            <ReactBoot.Navbar.Brand style={{fontSize:'x-large'}}>easyToll</ReactBoot.Navbar.Brand>
                            <ReactBoot.Nav style={{fontSize:'smaller', color:'#a6a6a6', marginTop:'1%', marginLeft:'-0.5%'}}>Dashboard</ReactBoot.Nav>
                            <ReactBoot.Navbar.Toggle aria-controls="basic-navbar-nav" />
                            <ReactBoot.Navbar.Collapse id="basic-navbar-nav">
                                <ReactBoot.Nav className="mr-auto"> </ReactBoot.Nav>
            ...
            return(
                <div style={{overflow:"hidden"}}>
                    ...
                    <div className="row" >
                        <div style={{width:"60%"}}>
                            <div ref={el => this.mapContainer = el} style={{height:'92vh'}}/>
                        </div>
                        <div style={{width:"2%"}} >
                            <button style={{width:"100%", height:"100%", backgroundColor:"#CACACA", border:'none'}} onClick={() => {                             
                                    if(this.state.selected!=null){
                                        this.setState({ trans : [], selected: null})}
                                    }
                                }>{">"}
                            </button>
                        </div>
                        <div className="collumn" style={{width:"38%",backgroundColor:"#F8F9FA", height:"92vh"}}>
                        <div >
                            <h4 style={{fontFamily:'Arial', marginTop:'10%'}}><b>Transactions Management</b></h4>
                            <h5 style={{fontFamily:'Arial', marginTop:'5%', marginLeft:'10%', textAlign: 'left'}}>Last passes by RSU number <strong>{this.state.selected}</strong> :</h5>
                        </div>
                        <div className="collumn" style={{height:"70vh" , overflow:"auto"}}>
                             <table >
                                <thead>
                                    <tr>
                                        <th>Date</th>
                                ...
                                <tbody>                               
                                    {this.state.trans.slice(0).reverse().map((d) =>{
                                        return(
                                            <tr>
                                                <td>{d.date}</td>
                                                <td>{d.clientID}</td>
                                                <td>{d.veichleID}</td>
                                                <td>{d.value} <i>iota</i></td>
                                                {d.address == "" ? (
                                                   <td style={{backgroundColor:"#FBC117"}}> </td>     
                                                ):(
